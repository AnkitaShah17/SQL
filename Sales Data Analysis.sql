...
FROM
    ...
    --returns 
returns_datareturns_datareturns_dataCREATE TABLE returns_data (
  order_id VARCHAR(20),
...
-- Top 10 sales -- 
SELECT 
    product_id, customer_name, city, sales
FROM
    orders_data
ORDER BY sales DESC
LIMIT 10;
...
FROM
    orders_data
GROUP BY order_date , category , product_id , customer_name
ORDER BY 'Total Profit' DESC;

-- Difference in delivery days for highest sales product and customers ---
SELECT 
    product_id,
    customer_name,
    quantity,
    sales,
    profit,
    DATEDIFF(ship_date, order_date) AS delivery_days_difference
FROM
    orders_data
ORDER BY sales DESC;
...
-- Total Sales by each region per year --
SELECT 
    region,
    SUM(sales) AS Total_sales,
    SUM(profit) AS 'Total_Profit',
...
-- Total Sales by each Category per year --
SELECT 
    city, category, SUM(profit) AS city_loss
FROM
    orders_data
GROUP BY city , category
HAVING city_loss <= 0;

-- Most profitable city and category -- 

SELECT 
    city, category, SUM(profit) AS Top_City
FROM
    orders_data
GROUP BY city , category
...
-- Distinct Values --
SELECT 
    COUNT(order_id) AS 'No of records',
    COUNT(DISTINCT customer_name) AS 'Distinct Customer names',
    COUNT(DISTINCT city) AS 'Distinct Cities',
    COUNT(DISTINCT category) AS 'No of Categories'
FROM
    orders_data;
...
FROM
    orders_data
        INNER JOIN
    returns_data ON orders_data.order_id = returns_data.order_id
GROUP BY category;

-- Sales for Bad Quality return --
...
FROM
    ...
    returns_data ON orders_data.order_id = returns_data.order_id
WHERE
    return_reason = 'Bad Quality'
GROUP BY city;

-- Profit for return_resons --

SELECT 
    r.return_reason, SUM(o.profit) AS 'Return Iteam Profit'
FROM
    orders_data o
        LEFT JOIN
...
SELECT 
    CASE
        WHEN r.return_reason = 'Wrong Items' THEN 'Wrong Item'
        ELSE r.return_reason
    END AS 'New Return Reason',
    SUM(o.profit) AS 'Return Iteam Profit'
FROM
    orders_data o
        LEFT JOIN
    returns_data r ON o.order_id = r.order_id
...
SELECT 
    *,
    CASE
        ...
        WHEN profit >= 1 AND profit <= 200 THEN 'low profit'
        WHEN profit >= 201 AND profit <= 500 THEN 'average profit'
        WHEN profit >= 501 THEN 'high profit'
    END AS 'Profit/Loss Bucket'
FROM
    orders_data;
    
    -- Country wise sales and Profit --
    
    select left(order_id, 2) as 'Country', sum(sales), sum(profit)
    from orders_data
